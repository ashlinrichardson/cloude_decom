...
int USE_PROPORTIONAL_SCALING = false; // default to proportional scaling
...
void two_percent(float & min, float & max, SA<float> * b){
  if(scene_band_min.count(b) < 1){
    ...
    unsigned int i;
    for(i = 0; i < b->size(); i += SCAN_INCREMENT){
      float dd = d[i];
      if(!(isinf(dd) || isnan(dd))) q.push(dd);
    }

    unsigned int n_pct = floor(0.01 * N_PERCENT_SCALING * ((float)q.size()));
...
void glImage::rebuffer(int recalc, int recall, int store){
  ...
  if(USE_PROPORTIONAL_SCALING){
	...
	printf("recalc..\n");
        ...
        if(store){
          MIN1 = min1; MIN2 = min2; MIN3 = min3; MAX1 = max1; MAX2 = max2; MAX3 = max3;
        }
      }
      else{
        if(recall){
  ...
  Update = false;
  printf("\tdone rebuffer MIN %f MAX %f MIN %f MAX %f MIN %f MAX %f\n", MIN1, MAX1, MIN2, MAX2, MIN3, MAX3);
}
...
void zprInstance::zprMouse(int button, int state, int x, int y){
 ...
 if(state == GLUT_DOWN){
     // 20210613 note: could add a virtual layer to the image, with the original indices (to simplify the reverse indexing to retrive image global target location) .. actually this was in progress.. see variable at top..
    if(true){
      std::vector<glPlottable *>::iterator it;
      int i = 0;
      it = myGraphics.begin();
    ...
    // printf("NR %d NC %d NB %d\n", (int)IMG_NR, (int)IMG_NC, (int)IMG_NB);
    fread(&(SCENE_DAT->elements[0]) , sizeof(float), IMG_NR * IMG_NC * IMG_NB, f);
    fclose(f);

    SCENE_MYIMG->initFrom(SCENE_DAT, IMG_NR, IMG_NC, IMG_NB);

//SCENE_MYIMG->rebuffer();
    glImage * image = (glImage *)SCENE_GLIMG;
    image->rebuffer(false, true, false);
    if(true){
    // print data under pixel again..
      std::vector<glPlottable *>::iterator it = myGraphics.begin();
      int i = 0;
      glImage * image = (glImage * )(glPlottable *) (*it);
      int NRow = image->image->NRow;
      int NCol = image->image->NCol;
