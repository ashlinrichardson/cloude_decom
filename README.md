# cloude_decom
High-performance interactive implementation of **Cloude Decomposition** [1] by [Shane R Cloude](https://scholar.google.ca/citations?hl=en&user=h-ZWMcUAAAAJ&view_op=list_works&sortby=pubdate). Expected input: 1) fully-polarimteric SAR data in standard [PolSARPro](https://ietr-lab.univ-rennes1.fr/polsarpro-bio/) T3 coherency matrix format with the usual **config.txt** file and 2) coordinates of user-selected target to "cancel". Output: *optimised radar cross section* (a greyscale image)
